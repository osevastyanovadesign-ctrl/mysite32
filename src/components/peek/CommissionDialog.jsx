...
// the request as a CommissionRequest record. The CTA that opens it lives
// in TierShowcase; this dialog never scrolls — it opens the order itself.
export default function CommissionDialog({ open, onClose, series }) {
  const { t } = useLang();
  const [form, setForm] = useState({ name: "", email: "", petName: "", message: "" });
  ...
  useEffect(() => {
    if (open) {
      setStatus("idle");
      setError("");
      setForm({ name: "", email: "", petName: "", message: "" });
      setPhoto(null);
      setPreview(null);
    }
  }, [open]);
            ...
            <div className="grid md:grid-cols-2">
              {status === "done" ? (
                <div className="text-center py-10">
                  ...
                  <p className="font-display text-3xl text-foreground mb-3">{t("commission.done")}</p>
                  <button
                    onClick={onClose}
                ...
                <>
                  ...
                  <p className="mt-3 text-muted-foreground leading-relaxed">{t("commission.subtitle")}</p>
                    ...
                    <div>
                      ...
                      <div className="mt-2 flex items-center gap-4">
                        <label className="cursor-pointer inline-flex items-center gap-2 rounded-full border border-border px-5 py-2.5 text-sm text-foreground hover:border-primary hover:text-primary transition-colors">
                          ...
                          <span>{photo ? photo.name : t("commission.photoHint")}</span>
